		...
		return this.each(function(){
			...
			if(keepSpans) {  //build array of sections to later slab
				...
				while(a=div.querySelectorAll("span.slabbedtext")[0]) {  // continues grabbing all span.slabbedtext tags until none are left:
					...
					sections.push({type: 1, value: groupedValue, raw: a.outerHTML });			// push the span code to the stack
					$(a).prevAll().remove();
					$(a).remove();
				}
				if(remainder != "") {
				...
				if(forceNewCharCount || fs != origFontSize) {
					...
					if(!keepSpans) {
						...
						for(var i = 0; i < sections.length; i++) {
							if(sections[i].type) {
								// TODO: keep id, class, and style attributes                                                              <-- WORK HERE
								finalSlabs.push(sections[i].raw);
							} else {
